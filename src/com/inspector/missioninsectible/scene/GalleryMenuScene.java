...
public class GalleryMenuScene extends Scene {
	...
	public GalleryMenuScene(){
		...
		if(!insectDb.isCatched(1)){
			 lockedBeetle = new Sprite(activity.getCameraWidth()/4 - 20, activity.getCameraHeight()/2 - 45, activity.greyBeetle, activity.getVertexBufferObjectManager());
		}else{
			lockedBeetle = new Sprite(activity.getCameraWidth()/4 - 20, activity.getCameraHeight()/2 - 45, activity.coloredBeetle, activity.getVertexBufferObjectManager());
		}
		lockedBeetle.setSize(30,30);
		attachChild(lockedBeetle);
		
		if(!insectDb.isCatched(2)){
			 lockedLadybug = new Sprite(activity.getCameraWidth()/4 + 15, activity.getCameraHeight()/2 - 45, activity.greyLadybug, activity.getVertexBufferObjectManager());
		}else{
			lockedLadybug = new Sprite(activity.getCameraWidth()/4 + 15, activity.getCameraHeight()/2 - 45, activity.coloredLadybug, activity.getVertexBufferObjectManager());
		}
		...
		if(!insectDb.isCatched(6)){
			lockedDragonfly = new Sprite(activity.getCameraWidth()/4 + 50, activity.getCameraHeight()/2 - 45, activity.greyDragonfly, activity.getVertexBufferObjectManager());
		}else{
			lockedDragonfly = new Sprite(activity.getCameraWidth()/4 + 50, activity.getCameraHeight()/2 - 45, activity.coloredDragonfly, activity.getVertexBufferObjectManager());
		}
		...
		if(!insectDb.isCatched(7)){
			lockedTimeInsect = new Sprite(activity.getCameraWidth()/4 +40, activity.getCameraHeight()/2 +30, activity.greyTimeInsect, activity.getVertexBufferObjectManager());
		}else{
			lockedTimeInsect = new Sprite(activity.getCameraWidth()/4 +40, activity.getCameraHeight()/2 +30, activity.coloredTimeInsect, activity.getVertexBufferObjectManager());
		}
		lockedTimeInsect.setSize(30,30);
		attachChild(lockedTimeInsect);
